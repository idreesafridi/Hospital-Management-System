                ...
                <div class="col-lg-8 col-md-12">
                    <div class="blog-view">
                        <div class="blog blog-single-post">
                            <div class="blog-image">
                                <a href="javascript:void(0);"><img alt=""
                                        src="{{ asset('backend') }}/{{ $blogs_tag->blogAttachment->file ?? 'blog_attachments/no_image.png' }}"
                                        class="img-fluid"></a>
                            </div>
                            <h3 class="blog-title">{{ $blogs_tag->title ?? 'N/A' }}</h3>
                            <div class="blog-info clearfix">
                                <div class="post-left">
                                    <ul>
                                        <li>
                                            <div class="post-author">
                                                <a href="{{ route('doctor_profile', $blogs_tag->doctorblogs->id) }}"><img
                                                        src="{{ asset('backend') }}/{{ $blogs_tag->doctorblogs->profile->profile_image ?? 'blog_attachments/no_image.png' }}"
                                                        alt="Post Author">
                                                    <span>Dr. {{ $blogs_tag->doctorblogs->name ?? 'N/A' }}</span></a>
                                            </div>
                                        </li>
                                        <li><i
                                                class="far fa-calendar"></i>{{ $blogs_tag->created_at->format(env('GLOBALE_DATE_FORMAT')) }}
                                        </li>
                                    ...
                                    <div class="about-author-img">
                                        <div class="author-img-wrap">
                                            <a href="{{ route('doctor_profile', $blogs_tag->doctorblogs->id) }}"><img
                                                    ...
                                                    src="{{ asset('backend') }}/{{ $blogs_tag->doctorblogs->profile->profile_image ?? 'blog_attachments/no_image.png' }}"></a>
                                        </div>
                                    </div>
                                    <div class="author-details">
                                        <a href="{{ route('doctor_profile', $blogs_tag->doctorblogs->id) }}"
                                            class="blog-author-name">Dr.
                                            {{ $blogs_tag->doctorblogs->name ?? 'N/A' }}</a>
                                        <p class="mb-0">{{ $blogs_tag->doctorblogs->profile->about ?? 'N/A' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="card blog-comments clearfix">
                            <div class="card-header">
                                <h4 class="card-title">Comments ({{ $blogs_tag->comments->count() ?? 0 }})</h4>
                            </div>
                            <div class="card-body pb-0">
                                <ul class="comments-list">
                                    <li>
                                        @foreach ($blogs_tag->comments as $comment)
                                            <div class="comment">
                        ...
                        <div class="card new-comment clearfix">
                            ...
                            <div class="card-body">
                                <form action="{{ route('comment') }}" method="POST" id="commentform">
                                    @csrf
                                    <input type="text" hidden name="blog_id" value="{{ $blogs_tag->id }}">
